...
const UserForUser = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (emailToSend) {
                try {
                    const result = await axios.post("http://localhost:3004/users/emailuser", {
                        email: emailToSend,
                    });
    ...
    const handleFileChange = async (event) => {
        ...
        if (file) {
            ...
            try {
                ...
                const response = await fetch("https://api.cloudinary.com/v1_1/dntrwijx5/image/upload", { method: "POST", body: data });
                const res = await response.json();
    
                if (res.secure_url) {
                    const emailToSend = auth && auth.email ? auth.email : user && user.email ? user.email : null;
    
                    if(emailToSend) {
                        try {
                            // Segunda solicitud: Enviar la URL de la imagen a tu backend junto con el email
                            const backendResponse = await axios.put("http://localhost:3004/users/editimage", {
                                email: emailToSend,
        ...
        <div>
            ...
            <div className="userContainer">
                <div className="imageContainer">
                    <img src={userData.image || "https://res.cloudinary.com/dntrwijx5/image/upload/v1695410025/imagenes/emnvfsjtizz9luh9ohpx.jpg"} alt={userData.name} className="userImage" />
                ...
                <div className="userName">
                    ...
                    <Tooltip title="Presiona para cambiar el nombre!" arrow>
                            ...
                            <EditIcon fontSize="small" color="primary" onClick={handleEditName} />
                    </Tooltip>
                </div>
                <h3>{userData.email}</h3>
                {/* ACA VA LA LISTA DE PRODUCTOS COMPRADOS */}
            </div>
            <FormDialog onClose={onClose} showEdit={showEdit} userData={userData} onUserUpdate={() => setUserUpdated(!userUpdated)} />
            <Footer />
        </div>
    );
};
