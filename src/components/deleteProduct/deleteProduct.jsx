...
const DeleteProduct = () => {
    ...
    const handleDelete = async (event) => {
        try {
            const deleteToggle = await axios.put(`${VITE_BACKEND_URL}/products/delete`, {productId: event.target.value});
            setUpdate(!update);
        } catch (error) {
            console.log(error);
        }
                ...
                <TableBody>
                    {products.map((product, index) => (
                        <TableRow key={index} style={{backgroundColor: index % 3 === 0 ? '#f5f5f5' : index % 3 === 1 ? '#e0e0e0' : '#c2c2c2'}}>
                            ...
                            <TableCell>
                                {product.isDeleted ? <Typography color="error">Eliminado</Typography> : <Typography color="success">Activo</Typography>}
                            </TableCell>
                            <TableCell>
                                <Button 
                                    variant="contained" 
