...
const DeleteUser = () => {
    ...
    const handleDelete = async (event) => {
        try {
            const deleteToggle = await axios.put(`${VITE_BACKEND_URL}/users/delete`, { userId: event.target.value });
            setUpdate(!update);
        } catch (error) {
            console.log(error);
        }
            ...
            <Table size="small">
                ...
                <TableBody>
                    {users.map((user, index) => (
                        <TableRow key={index} style={{ backgroundColor: index % 3 === 0 ? '#f5f5f5' : index % 3 === 1 ? '#e0e0e0' : '#c2c2c2' }}>
                            <TableCell>{user.id}</TableCell>
                            <TableCell>{user.name}</TableCell>
                            <TableCell>{user.email}</TableCell>
                            <TableCell>
                                {user.isDeleted ? <Typography color="error">Eliminado</Typography> : <Typography color="success">Activo</Typography>}
                            </TableCell>
                            <TableCell>
                                {user.confirmated ? <Typography color="error">No confirmado</Typography> : <Typography color="success">Confirmado</Typography>}
                            </TableCell>
                            <TableCell>
                                <Button 
                                    variant="contained" 
